...
const AdminItems = () => {
  ...
  const handleBulkMoveToRentals = async (action: 'move' | 'copy') => {
    ...
    try {
      if (action === 'move') {
        ...
        // Copy: fetch selected items, then insert duplicates with is_rental=true
        ...
        toast({ title: `${selectedItems.length} item(s) copied to Rentals!` });
      }
      setSelectedItems([]);
  ...
  return (
    <div className="space-y-8 animate-fade-in">
          ...
          {selectedItems.length > 0 && (
            <>
              ...
              <Button variant="outline" onClick={() => setBulkMoveDialogOpen(true)}>
                ...
                Move / Copy to Rentals ({selectedItems.length})
              </Button>
            </>
          )}
      ...
      <BulkMoveDialog
        ...
        from="Items"
        to="Rentals"
        onConfirm={handleBulkMoveToRentals}
